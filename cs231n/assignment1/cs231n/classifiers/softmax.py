...
def softmax_loss_naive(W, X, y, reg):
    ...
    # Softmax Loss
    for i in range(num_train): # train 개수 만큼
        f = scores[i] - np.max(scores[i]) # avoid numerical instability
        # max값 빼서 부동소수점 연산 줄이게 해줌
        softmax = np.exp(f)/np.sum(np.exp(f)) # softmax로 만들어줌
        loss += -np.log(softmax[y[i]]) #loss Function
        # Weight Gradients
        for j in range(num_classes):
            dW[:,j] += X[i] * softmax[j] # dW 는 dLdW = dLds * X[i]이다
            dW[:,y[i]] -= X[i] # 가중치 업데이트 W := W -a * dLdw
    # Average
    loss /= num_train
    dW /= num_train
    # Regularization
